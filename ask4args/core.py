...
from typing import Any, Dict, List, _alias, _GenericAlias
...
class BaseSchema(object):
    ...
    def run(self):
        ...
        self.function(*args['varargs'], **kwargs)
...
[summary]
    ...
    def check_type(self, value, value_type=Any):
    ...
    def make_question(self, arg) -> Dict:
        ...
        if origin_type in {int, float}:
            ...
            if arg.default != ...:
                raise ValueError('type [list, tuple, dict] can not set default value.')
            if self.use_raw_list:
                question['type'] = 'rawlist'
            else:
                question['type'] = 'list'
            if arg.name in self.choices:
                question['choices'] = self.choices[arg.name]
            elif arg.name in self.checkboxes:
                question['choices'] = [{
                    'name': item
    ...
    def ask_for_args(self):
        ...
        print(
            f'function({self.function.__name__})[rtype: {args["kwargs"].pop("return", Schema(0,Any,0)).type}] starts asking questions:'
        )
        self._kwargs = {'kwargs': {}}
        questions = [{
            'type': 'confirm',
        ...
        for index, item in enumerate(args['kwargs'].items()):
            name, arg = item
            if index == 0 and arg.default == ... and inspect.ismethod(
                    self.function):
                # ignore method's cls / self
                continue
            elif arg.name in self.defaults:
                # use default value instead
                self._kwargs[name] = self.defaults[arg.name]
            else:
    ...
    def make_schema(self, function) -> Dict[str, Any]:
        ...
        if args.varargs is not None:
            kwargs[args.varargs] = Schema(
                args.varargs,
                self.ensure_type(args.annotations.get(args.varargs, List[str])),
                ...)
        # process the **varkw
        ...
        kwargs['return'] = Schema(
            'return', self.ensure_type(args.annotations.get('return', Any)),
            ...)
        valid_types = {list, int, bool, str, tuple, set, float, dict}
        for arg in kwargs.values():
            if arg.type.__origin__ not in valid_types:
                raise TypeError(f'arg type only support the {valid_types}')
